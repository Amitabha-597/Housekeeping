...
Page({
  ...
  seaveAdd(e) {
    app.$request('/personal/editAddress', 'POST', this.data.newaddFrom).then(res => {
      console.log(res)
     if(res.code==1){
  ...
  onLoad: function (options) {
    let id=options.addres
    console.log(id);
    
    app.$request('/personal/editAddress', 'GET', {id:id}).then(res => {
      let addlist=[]
      addlist.push(res.data.province,res.data.city,res.data.area)
      this.setData({
        newaddFrom:res.data,
        region:addlist
      })
